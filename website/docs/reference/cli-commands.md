...
| `resolve-block-loop <id> <decision>` | Resolve a `block_loop_detected` triage task with `retry`, `complete`, or `archive`; requires `--reason`, and `complete` also requires `--summary` or `--result`. |
