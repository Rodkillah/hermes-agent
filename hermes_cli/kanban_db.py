...
_TASK_OPTIONAL_COLUMNS = (
    ...
    # Retained when a delivered item is routed back to todo solely because its
        ...
        try:
            ...
            with write_txn(conn, allow_nested=True):
                ...
                if task_status == "todo":
                    ...
                    gating = [p for p in parents if _parent_still_gates(conn, p)]
                    if gating:
                        _append_event(
                            conn,
                            task_id,
...
def _parent_still_gates(conn: sqlite3.Connection, parent_id: str) -> bool:
    """True when ``parent_id`` has not delivered and therefore still gates its
    children. Row-level twin of :func:`_parents_satisfied`'s SQL predicate, so
    the link-time surfaces (``create_task``, :func:`link_tasks`) and the
    dispatcher answer the same question the same way.

    Iron Rod contract: only a real delivery releases a descendant - ``done``,
    ``prod``, or a delivered card parked back in ``todo`` awaiting deployment.
    ``archived`` is NOT a delivery and keeps blocking. An unknown parent gates
    (fail-closed).

    DECLARED DRIFT (Iron Rod, merge of Sep 2026). This is NOT a byte-for-byte
    port of Iron Rod's previous link-time predicate, which read
    ``status NOT IN (done, prod) AND work_completed_at IS NULL``. The two
    answers differ for a parent carrying a completion stamp in a status that is
    neither ``todo`` nor terminal - ``ready``, ``running``, ``review`` - a state
    ``route_done_to_deploy_todo`` creates (``todo`` + ``work_completed_at``) and
    ``recompute_ready`` then promotes to ``ready``. The old predicate released
    such a child at link time while :func:`_parents_satisfied` kept gating it,
    so the board advertised a ``ready`` card the dispatcher would never claim.
    The tightening is deliberate and fail-closed: one question, one definition.
    Covered by ``tests/hermes_cli/test_kanban_parent_still_gates.py``, which
    turns red if this predicate drifts back to either the old Iron Rod form or
    upstream's, and which asserts helper == dispatcher on every status.
    """
    row = conn.execute(
    ...
    if row is None:
        return True
    return (
        row["status"] not in WORK_COMPLETED_STATUSES
        and not (row["status"] == "todo" and row["work_completed_at"] is not None)
    )
...
def link_tasks(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        # If child was ready but the parent has not delivered, demote to todo.
        # A parent in ``prod``, or parked back in ``todo`` awaiting deployment,
        # has completed its work and must not demote its children. Iron Rod
        # contract: ``archived`` is NOT a delivery, so an archived parent keeps
        # gating (same predicate as ``_parents_satisfied``).
        if _parent_still_gates(conn, parent_id):
            cur = conn.execute(
...
def recompute_ready(conn: sqlite3.Connection, failure_limit: int = None) -> int:
    """Promote ``todo``/``blocked`` tasks whose parents have all delivered
    (Iron Rod contract: ``done``/``prod``, or a card parked back in ``todo``
    with ``work_completed_at`` set; ``archived`` does NOT release a
    descendant, see :func:`_parents_satisfied`);
    ...
    IRON-ROD-OUT-OF-CONFLICT (merge of Sep 2026): the first sentence used to
    read "parents are all done/archived" in both branches while the body below
    already carried the Iron Rod predicate. Docstring only, no behaviour change;
    it is corrected here so the contract is not documented as its opposite.
    """
...
def _parents_satisfied(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    return conn.execute(
        ...
        "WHERE l.child_id = ? "
        # Iron Rod contract: ``archived`` does NOT release descendants, only a
        # real completion does. ``prod``, and a delivered card parked back in
        # ``todo`` awaiting deployment, both count as work completed.
        "AND p.status NOT IN ('done', 'prod') "
        "AND NOT (p.status = 'todo' AND p.work_completed_at IS NOT NULL) LIMIT 1", (task_id,),
    ).fetchone() is None
...
def unsatisfied_parents(conn: sqlite3.Connection, task_id: str) -> list[tuple[str, str]]:
    ...
    still counts as open, in id order, so a refusal or a board view can name the
    blockers instead of the caller guessing. Read-only.

    The predicate is copied from :func:`_parents_satisfied` on purpose: one
    question, one definition. Iron Rod contract, so ``archived`` is NOT a
    release and an archived parent is named here like any other blocker.

    IRON-ROD-OUT-OF-CONFLICT (merge of Sep 2026). This function is purely
    upstream (commit 0cdc325b46) and its body raised NO merge conflict: the
    predicate below was rewritten by Iron Rod, not resolved. Reason: upstream
    shipped ``NOT IN (done, archived)``, which names a different blocker set
    than the dispatcher applies, so a dependency refusal would have listed
    parents that do not gate and stayed silent about an archived one that does.
    Commit and review this as its own Iron Rod patch, not as merge fallout.
    """
    rows = conn.execute(
        ...
        "WHERE l.child_id = ? "
        "AND p.status NOT IN ('done', 'prod') "
        "AND NOT (p.status = 'todo' AND p.work_completed_at IS NOT NULL) "
...
def promote_task(
    ...
    unsatisfied = [p["id"] for p in parents if p["status"] not in WORK_COMPLETED_STATUSES]
    if unsatisfied:
        return False, (
    ...
    with write_txn(conn, allow_nested=True):
        ...
        for row in rows:
            ...
            conn.execute(
                ...
                "work_completed_at = NULL, claim_lock = NULL, claim_expires = NULL, "
                "worker_pid = NULL, worker_started_at = NULL, "
...
def archive_task(conn: sqlite3.Connection, task_id: str, *, signal_fn=None) -> bool:
    ...
    # parent does NOT release its descendants. Only a real completion does.
    # Promote whatever the archive genuinely unblocked.
    recompute_ready(conn)
...
def gc_events(conn: sqlite3.Connection, *, older_than_seconds: int = 30 * 24 * 3600) -> int:
    ...
    with write_txn(conn):
        cur = conn.execute(
            ...
            "AND kind != 'production_promoted'", (cutoff,),
        )
