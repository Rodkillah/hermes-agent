...
_SPECS = [
    ...
    _cmd("resolve-block-loop", [
        _TASK_ID,
        _arg("decision", choices=("retry", "complete", "archive")),
        _arg("--reason", required=True, help="Durable reason for the human decision."),
        _arg("--actor", help="Decision maker (defaults to the active profile)."),
        _arg("--expected-event-id", type=int, required=True,
             help="Current block_loop_detected event id from a fresh task read (CAS guard)."),
        _arg("--summary", help="Required handoff for complete."),
        _arg("--result", help="Optional result text for complete."),
        _arg("--metadata", help="JSON object for the completion handoff."),
    ], help="Resolve a block-loop triage task with retry, complete, or archive"),
    _cmd("mark-prod", [
        _TASK_ID,
        _arg("--receipt-file", required=True,
    ...
    _cmd("route-deploy-todo", [
        _TASK_ID,
        _arg("--candidate-sha", required=True),
        _arg("--audit-id", required=True),
        _arg("--reason", required=True),
        _arg("--next-action", required=True),
