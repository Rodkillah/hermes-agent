...
KANBAN_GUIDANCE = (
    ...
    "reviewer=<optional-profile>)`. A source reviewer never completes that "
    "card directly: GO uses `kanban_approve_production` with an explicit safe "
    "risk classification, which moves it to `production_ready` and restores "
    "the implementer. Credentials, secrets, destructive data work, money, "
    "legal, architecture decisions, and external actions must instead be a "
    "distinct `blocked` human gate with no assignee. The production worker "
    "deploys the exact candidate and calls `kanban_mark_prod_implemented` "
    "with version, timestamp, canary, main promotion, backup, and prepared "
    "rollback evidence; "
    "Architect then independently verifies live probes and rollback from "
    "`prod_implemented`. Only that live review may call `kanban_complete`, "
    "with metadata containing `delivery_level=verified_production`, "
    "`production_version`, `deployed_at`, `post_deploy_checks`, and "
    "`rollback`. Any source or live NO-GO uses `kanban_request_changes`. "
    "Review is not a block, so repeated review cycles do not trip "
