...
- Rejeu historique R10→R15 avec les adaptateurs de seam R14 effectifs (aucune désélection) → 87 passed, rc 0.
...
3. Backup SQLite ciblé avec `VACUUM INTO` et `quick_check`; sauvegarder les quatre fichiers ciblés (`hermes_cli/kanban_db.py`, `cron/jobs.py`, `cron/scheduler.py` et le script Amber) avec hash/mode et pré-images sur le même système de fichiers.
