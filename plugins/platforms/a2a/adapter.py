...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        ...
        with self._forward_lock(key):
            ...
            # Iron Rod : le tour forwarde n herite ni du marqueur de gateway, ni de l identite
            # de worker du dispatcher. Le finalizer d env REINJECTE HERMES_SESSION_* depuis les
            # ContextVars de la session gateway, et scrub_kanban_env n efface que les cinq cles
            # HERMES_KANBAN_* enumerees (ni _DB, ni _BOARD) : on purge les deux prefixes en
            # entier apres coup. La session cible est deja designee par --resume/--source.
            # NE PAS nommer cette variable ``key`` : elle masquerait la cle du cache de
            # sessions calculee plus haut, et la session forwardee serait memorisee sous un
